...
data_path = "data/Test_4_Usoskin.mat"; %% Insert the path to data in the quotes
...
[S00, y_simlr, S_simlr, F_simlr, ydata_simlr,~,~,~,LF_simlr] = SIMLR(X,5,10); %% without graph diffusion
[S000, y_simlr_diff, S_simlr_diff, F_simlr_diff, ydata_simlr_diff, ~, ~,~,LF_simlr_diff] = SIMLRv2(X,5,10); %% with graph diffusion
...
% 
% %% Spectral Clustering
% [y_spec, V_spec] = spectralcluster(S00,5,'Distance','precomputed');
% sil_SC = mean(silhouette(V_spec,y_spec));
% 
% %% Spectral Clustering with Graph Diffusion
% [y_spec_diff, V_spec_diff] = spectralcluster(S000,5,'Distance','precomputed');
...
% writematrix(y_kmeans, output_folder+'/clusters_labels_kmeans_.txt')
